    ...
    public void  drawGraph(Graphics g)
    {
        ...
        if (order > 0)
        {
            for (j = 0; j < order; j++)
            {
                ...
                p1.y = (pGraph.getHeight() / 2) - (int)Math.round(dft.y[j].real * 5.0);
                p2.x = (int)Math.round(j / 2 + 1);
                p2.y = (pGraph.getHeight() / 2) - (int)Math.round(dft.y[j + 1].real * 5.0);
    ...
    private void SelectionChanged()
    {
    	...
    	break;
         ...
         for (j = 0; j < 30; j++)        // put values into the DataGrid
         {
              ...
              str = Double.toString(Math.round(dft.c[j].real*10000)/10000.0);
              tableData.setValueAt(str, j, 1);
              str = Double.toString(Math.round(dft.c[j].imag*10000)/10000.0);
