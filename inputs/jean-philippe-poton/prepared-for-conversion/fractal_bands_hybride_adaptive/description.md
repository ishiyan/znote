...
|-----------|------|---------|-------|-------------|
| `period` | int | 30 | [2, len(prices)] | Lookback period for FDI computation |
| `normal_speed_fallback` | int | 30 | [1, len(prices)] | Fallback SMA period (used if CyclePeriod unavailable) |
| `alpha` | float | 2.0 | (0, inf) | Band width multiplier (raised to power $H$) |
| `nyquist` | float | 0.5 | (0, inf) | Nyquist multiplier applied to the estimated cycle period |
| `alpha_hp` | float | 0.07 | (0, 1) | High-pass filter alpha for Ehlers CyclePeriod |
...
| `upper` | Upper band = frasma + deviation * alpha^H |
| `lower` | Lower band = frasma - deviation * alpha^H |

## Test Data Parameter Combinations

16 combinations total: period in {10, 20, 30, 50} x nyquist in {0.5, 1.0} x alpha_hp in {0.07, 0.15}.
...
```mermaid
flowchart TD
    A[Input: price array, period, alpha, nyquist, alpha_hp] --> B[For each bar pos]
    B --> C[Compute FDI over period window]
    C --> D["H = 2 - FDI"]
