...
Note: The original MQ4 `type_data` parameter (price type selector) is not
included here since the conversion operates on a single input price array.

## Outputs

| Output | Description |
|--------|-------------|
| `hurst_diff` | First difference of FDI. First `period + 1` values are NaN. |
| `fdi` | Raw FDI values (corrected FGDI formula). First `period` values are NaN. |

## Test Parameters

8 parameter sets × 2 outputs = 16 expected arrays:

| `period` | FDI NaN count | Hurst Diff NaN count |
|----------|---------------|----------------------|
| 5 | 5 | 6 |
| 10 | 10 | 11 |
| 15 | 15 | 16 |
