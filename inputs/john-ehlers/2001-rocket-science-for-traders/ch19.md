...
Once the filter has been set, we can do several things. First, we can connect a sweep generator to the filter input and sense the relative amplitude of the output as the frequency band is swept. This produces the bell-shaped spectral estimate similar
...
*Figure 19.6. MESA spectrum measurement of March 1996 Treasury*
...
m The Fast Fourier Transform (FFT) is not the proper tool to analyze market data.

m An FFT can measure only an integer number of cycles within its observation window.

m An FFT requires a large amount of data to achieve high-resolution measurements. If we are looking at market data over a long time span, the FFT is useless because the data cannot fulfill the requirement to remain relatively stationary in order to achieve a valid measurement.

m MESA operates by pattern matching in the time domain. Data outside the short observation window are rejected.

m There is typically only one dominant cycle in the market at a time.
