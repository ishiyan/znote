...
namespace NinjaTrader.Indicator
{
    ...
    public class SwamiStochastics : Indicator
    {
		...
		private Color bulishColor	= Color.Green;
        ...
        protected override void OnBarUpdate()
        {
			...
			for(int x = 0; x < iValues.Length; x++)
				iValues[x].Set(Stochastics(3, x + minLength, 3)[0] * 0.01);
        }
        ...
        public int MinLength
        {
            ...
            set { minLength = Math.Min(Math.Max(6, value), 24); }
        }
        ...
        public int MaxLength
        {
            ...
            set { maxLength = Math.Min(Math.Max(24, value), 48); }
        }
...
namespace NinjaTrader.Indicator
{
    ...
    {
        private SwamiStochastics[] cacheSwamiStochastics = null;

        private static SwamiStochastics checkSwamiStochastics = new SwamiStochastics();

        /// <summary>
        /// Stocks and Commodities - March 2012 - Introducing SwamiCharts
        /// </summary>
        /// <returns></returns>
        public SwamiStochastics SwamiStochastics(Color bearishColor, Color bulishColor, int maxLength, int minLength, Color neutralColor)
        {
            return SwamiStochastics(Input, bearishColor, bulishColor, maxLength, minLength, neutralColor);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public SwamiStochastics SwamiStochastics(Data.IDataSeries input, Color bearishColor, Color bulishColor, int maxLength, int minLength, Color neutralColor)
        {
            if (cacheSwamiStochastics != null)
                for (int idx = 0; idx < cacheSwamiStochastics.Length; idx++)
                    if (cacheSwamiStochastics[idx].BearishColor == bearishColor && cacheSwamiStochastics[idx].BulishColor == bulishColor && cacheSwamiStochastics[idx].MaxLength == maxLength && cacheSwamiStochastics[idx].MinLength == minLength && cacheSwamiStochastics[idx].NeutralColor == neutralColor && cacheSwamiStochastics[idx].EqualsInput(input))
                        return cacheSwamiStochastics[idx];

            lock (checkSwamiStochastics)
            {
                checkSwamiStochastics.BearishColor = bearishColor;
                bearishColor = checkSwamiStochastics.BearishColor;
                checkSwamiStochastics.BulishColor = bulishColor;
                bulishColor = checkSwamiStochastics.BulishColor;
                checkSwamiStochastics.MaxLength = maxLength;
                maxLength = checkSwamiStochastics.MaxLength;
                checkSwamiStochastics.MinLength = minLength;
                minLength = checkSwamiStochastics.MinLength;
                checkSwamiStochastics.NeutralColor = neutralColor;
                neutralColor = checkSwamiStochastics.NeutralColor;
                ...
                SwamiStochastics indicator = new SwamiStochastics();
                ...
                SwamiStochastics[] tmp = new SwamiStochastics[cacheSwamiStochastics == null ? 1 : cacheSwamiStochastics.Length + 1];
                if (cacheSwamiStochastics != null)
                    cacheSwamiStochastics.CopyTo(tmp, 0);
                tmp[tmp.Length - 1] = indicator;
                cacheSwamiStochastics = tmp;
                return indicator;
            }
        }
    }
...
namespace NinjaTrader.MarketAnalyzer
{
    public partial class Column : ColumnBase
    {
        ...
        public Indicator.SwamiStochastics SwamiStochastics(Color bearishColor, Color bulishColor, int maxLength, int minLength, Color neutralColor)
        {
            return _indicator.SwamiStochastics(Input, bearishColor, bulishColor, maxLength, minLength, neutralColor);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Indicator.SwamiStochastics SwamiStochastics(Data.IDataSeries input, Color bearishColor, Color bulishColor, int maxLength, int minLength, Color neutralColor)
        {
            return _indicator.SwamiStochastics(input, bearishColor, bulishColor, maxLength, minLength, neutralColor);
        }
    }
