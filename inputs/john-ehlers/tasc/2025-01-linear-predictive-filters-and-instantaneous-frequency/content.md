...
- **Publication:** Technical Analysis of STOCKS & COMMODITIES, Volume 43, January 2025, pp. 8–18
- **Article URL:** https://technical.traders.com/archive/article.asp?file=\V43\C01\898EHLE.pdf
- **Traders' Tips URL:** https://www.traders.com/Documentation/FEEDbk_docs/2025/01/TradersTips.html
...
SPOILER ALERT: We are going to take a relatively deep dive into DSP (digital signal processing) in this article. But the dive will be worth it because it will answer the most vexing question of technical traders. That question is: "Why did my technical indicator or algorithmic trading rule suddenly fail after looking so good in backtest?"

The short answer is that the indicators and rules have static parameters, but the price data has a dynamic time variation. The price data has a complex waveform in the time domain.

From Fourier theory, the complex waveform can equally well be described in the frequency domain. Figure 1 shows approximately one year of the emini S&P futures contract, with the measured spectrum in the frequency domain in the first subgraph. The wavelength of spectral components is displayed along the vertical axis of the subgraph, and the amplitude of the spectral components are shown in color—from white hot, through red hot, down to ice cold in black. The spectrum display is in time sync with the price chart. From an overview perspective, we see the market had a 32-bar dominant cycle period in fall 2023 that radically changed to a 25-bar dominant cycle period in the runup of the winter and spring of 2024. There was a shift of the dominant cycle period in May, and yet another major shift in July.

![Figure 1: Measured Spectrum](assets/figure-1-measured-spectrum.png)

**FIGURE 1: MEASURED SPECTRUM.** Shown here is approximately one year of the emini S&P futures contract, with the measured spectrum in the frequency domain in the first subgraph. The wavelength of spectral components is displayed along the vertical axis of the subgraph. The measured spectrum shows major shifts in the dominant cycle in price data. Unless your technical analysis indicator can accommodate or adapt to major shifts such as these, the indicator will fail to keep working.

Unless your indicator could accommodate or adapt to these shifts, it is doomed to failure. A typical technical analysis approach is to use one kind of indicator until it fails—then adjust the parameters or use another indicator to fit the new market conditions. One of the results of our dive will be to recognize and adjust to the market changes as they occur.
...
So, in a nutshell, we want to solve the drunkard's walk problem with Z transforms because we are dealing with sampled data. Price and its rate of change are about all we have to work with. The use of volume to predict price is demonstrably worthless, at least with intraday data. Using patterns is subjective and the statistics on the success of using patterns for price prediction is abysmal. In my opinion, the use of numerology, position of the planets, and the like is just black art.
...
A simple predictive filter is coded in the sidebar below. At zero frequency $Z^{-1} = 1$, and we want to have unity gain at this frequency. Therefore, each of the coefficients must be normalized to their sum for this to happen. You can uncomment one line of code to get a deterministic sinewave having a 20-bar wavelength and its prediction. You can also play with the value of Q. If you make Q too small, there is not much of a prediction. If you make Q too large (but less than 1), the filter becomes a peaking filter and the prediction becomes erratic. Of course, the prediction can be extended by making the first prediction be a new data point and then running the process again. So, making a predictive filter is simple. The trick is how to compute the coefficients.

### A Simple 2-Pole Predictor (EasyLanguage)
...
### Highpass Function (EasyLanguage)
...
### SuperSmoother Function (EasyLanguage)
...
### Griffiths Predictor (EasyLanguage)

```easylanguage
{
    Griffiths Predictor Indicator
    (C) 2024 John F. Ehlers
    from "Rapid Measurement of Digital Instantaneous Frequency", IEEE Transactions ASSP-23
}
Inputs:
    LowerBound(18),
    UpperBound(40),
...
![Figure 2: Predictions](assets/figure-2-predictions.png)

**FIGURE 2: PREDICTIONS.** Here you see swing trading predictions for approximately one year of the emini S&P. The prediction is shown in blue and the band-limited and normalized data is shown in red for 2 BarsFwd. In general, the prediction crossing the signal line can provide excellent timing for swing trading entries and exits.

Figure 2 shows the prediction in blue and the band-limited and normalized data in red for 2 BarsFwd. The lower bound was set at 18, and the upper bound and data length were set at 40. In general, the prediction crossing the signal line provides excellent timing for swing trading entries and exits.
...
The EasyLanguage code to compute the Griffiths spectrum is given below. Finding the coefficients is done exactly as was done for the Griffiths predictor. The arrays are scaled at 100 because the indicator is constrained to have no more than 99 indicator lines.
...
For Period = LowerBound to UpperBound Begin
  ...
  If Pwr[Period, 1] < .5 Then Begin
    ...
    Color2 = 0;
  End;
  // Plot statements for each period (3 through 99)
  // e.g.: If Period = 10 Then Plot10(10, "S10", RGB(Color1, Color2, 0), 0, 4);
...
![Figure 3: Griffiths Dominant Cycle](assets/figure-3-dominant-cycle.png)

**FIGURE 3: GRIFFITHS DOMINANT CYCLE.** The data dominant cycle extracted from the Griffiths spectrum is displayed as an indicator. Here you see the dominant cycle for the emini S&P over approximately the last year and cycle period range between 18 and 40 bars.
...
- Lloyd J. Griffiths, "Rapid Measurement of Digital Instantaneous Frequency," *IEEE Transaction ASSP-23*, pp. 187–202.
...
@misc{traderstips2025jan,
  title     = {Traders' Tips: Linear Predictive Filters and Instantaneous Frequency},
  year      = {2025},
  month     = jan,
  howpublished = {Technical Analysis of Stocks \& Commodities, Volume 43, Number 1},
  url       = {https://www.traders.com/Documentation/FEEDbk_docs/2025/01/TradersTips.html},
  note      = {Implementations of Ehlers' technique in various trading platforms}
}
```
