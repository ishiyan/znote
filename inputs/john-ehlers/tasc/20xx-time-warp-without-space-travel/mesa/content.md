...
- **Downloaded from:** [Mesa Software — Time Warp](https://www.mesasoftware.com/papers/TimeWarp.pdf)
...
![Figure 1: Schematic of a Moving Average](assets/fig-01.png)
...
![Figure 2: Schematic of a Four Element FIR Filter](assets/fig-02.png)
...
![Figure 3: Four Bar Moving Average Frequency Response](assets/fig-03.png)
...
![Figure 4: Four Element FIR Filter Frequency Response](assets/fig-04.png)
...
The Laguerre Transform can be represented as a EMA low pass filter (the first term),

             1 - γ      ⎡ Z^-1 - γ  ⎤^(k-1)
    H(z) = -----------  ⎢ --------- ⎥
            1 - γ Z^-1  ⎣ 1 - γ Z^-1⎦

followed by a succession of all-pass elements instead of unit delays (the k-1 terms). All terms have exactly the same damping factor γ. We see these are all pass networks by examining the frequency response. When frequency is zero, the Z^-1 term has a value of 1, and therefore the element evaluates to (1-γ)/(1-γ) = 1. Similarly, when frequency is infinite, Z^-1 has a value of -1, and therefore the element evaluates to (-1-γ)/(1+γ) = -1. The element has a unity gain at all frequencies between zero and infinity, and therefore is an all pass network. However, the phase from input to output shifts over the frequency range, causing the lag to be variable as a function of frequency. The degree to which the lag is variable depends upon the value of the damping factor γ. For example, the lag, or group delay, for γ = 0.6 and γ = 0.8 are shown in Figure 5.

![Figure 5: All Pass Network Lag](assets/fig-07.png)
...
![Figure 6: Laguerre Filter Schematic](assets/fig-06.png)
...
### Code Listing 1 (Figure 7). Laguerre Filter EasyLanguage Code
...
![Figure 8: Four Element Laguerre Filter is Dramatically Smoother](assets/fig-08.png)
...
### Code Listing 2 (Figure 9). EasyLanguage Code for a Laguerre RSI Indicator
...
![Figure 10: A Laguerre RSI Reacts Rapidly to Price Changes](assets/fig-09.png)
...
![Figure 11: Screen Capture from www.wellinvested.com](assets/fig-11.png)
...
```bibtex
@misc{ehlers_time_warp,
  author       = {John F. Ehlers},
  title        = {Time Warp -- Without Space Travel},
  year         = {2002},
  howpublished = {online},
  url          = {https://www.mesasoftware.com/papers/TimeWarp.pdf}
}
```
