"""Generate test_testdata.py with 15-decimal precision reference outputs.

Reads INPUT_CLOSE from new-moving-average.py's __main__ block and computes NMA
for all parameter combinations, writing results to test_testdata.py.

Usage: python gen_test_testdata.py
"""
...
nma_mod = SourceFileLoader("nma", os.path.join(os.path.dirname(__file__), "new-moving-average.py")).load_module()

nyquist_ma = nma_mod.nyquist_ma
MAType = nma_mod.MAType
...
def main():
    ...
    outpath = os.path.join(os.path.dirname(__file__), "test_testdata.py")
    ...
    print(f"Generated test_testdata.py with {len(test_cases)} arrays at 15-decimal precision")
