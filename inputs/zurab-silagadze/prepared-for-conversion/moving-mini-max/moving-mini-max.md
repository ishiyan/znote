...
| `with_distribution` | Whether to include the full probability distribution arrays in output. | True (batch) / False (streaming) | bool |
...
| Output | Description |
|--------|-------------|
| `up` | Up mini-max value at the most recent bar (scalar float). |
| `down` | Down mini-max value at the most recent bar (scalar float). |
| `resistances` | List of up to `num_extrema` distinct resistance levels, sorted by strength (strongest first). |
| `supports` | List of up to `num_extrema` distinct support levels, sorted by strength (strongest first). |
| `up_distribution` | Full n-length up mini-max probability distribution array (sums to 1.0). Empty list if `with_distribution=False`. |
| `down_distribution` | Full n-length down mini-max probability distribution array (sums to 1.0). Empty list if `with_distribution=False`. |
...
The distribution arrays are probability distributions: each element is non-negative and the sum of all elements equals 1.0. Higher values indicate stronger extrema at that position.

### Streaming Mode (not primed)

When using the streaming API and the indicator has not yet received `n` bars:
- `up` and `down` are set to `NaN`
- `resistances` and `supports` are empty lists
- `up_distribution` and `down_distribution` are empty lists
- `primed` is `False` (streaming output only)
