...
def moving_minimax(prices, m=5, n=300, num_extrema=3, with_distribution=True):
    """
    ...
    with_distribution : bool, optional
        If True (default), include the full n-length probability distribution
        arrays in the output. If False, return empty arrays instead.

    Returns
    -------
    dict with keys:
        'up' : float
            Up mini-max value at the most recent bar (last position in window).
        'down' : float
            Down mini-max value at the most recent bar.
        'resistances' : list of dict
            Top num_extrema resistance levels, each with:
                ...
                'strength': float — up-minimax value at that peak
        ...
        'up_distribution' : list of float
            Full n-length up mini-max probability distribution (sums to 1.0).
            Empty list if with_distribution=False.
        'down_distribution' : list of float
            Full n-length down mini-max probability distribution (sums to 1.0).
            Empty list if with_distribution=False.
    ...
    up_dist = calc_minimax(p_up_plus, p_up_minus, n)
    dn_dist = calc_minimax(p_dn_plus, p_dn_minus, n)
    ...
    u_peaks = find_peaks(up_dist, num_extrema, min_sep)
    d_peaks = find_peaks(dn_dist, num_extrema, min_sep)
    ...
    return {
        ...
        'supports': supports,
        'up_distribution': up_dist if with_distribution else [],
        'down_distribution': dn_dist if with_distribution else [],
    }
...
if __name__ == '__main__':
    ...
    # Latest bar values
    print(f"  Latest bar: up={result['up']:.6f}, down={result['down']:.6f}")
    print()

    # Verify normalization (distribution included by default)
    u_sum = sum(result['up_distribution'])
    d_sum = sum(result['down_distribution'])
    print(f"  up_distribution sum:   {u_sum:.6f} (should be 1.0)")
    print(f"  down_distribution sum: {d_sum:.6f} (should be 1.0)")
    print(f"  up_distribution length:   {len(result['up_distribution'])}")
    print(f"  down_distribution length: {len(result['down_distribution'])}")
    print()
    ...
    # Demonstrate with_distribution=False
    print()
    print("  --- with_distribution=False ---")
    result2 = moving_minimax(prices, m=m, n=n, num_extrema=3,
                             with_distribution=False)
    print(f"  up_distribution length: {len(result2['up_distribution'])} (empty)")
    print(f"  up={result2['up']:.6f}, down={result2['down']:.6f}")
    print(f"  resistances: {len(result2['resistances'])}")
